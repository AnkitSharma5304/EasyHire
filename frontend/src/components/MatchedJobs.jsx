...
const MatchedJobs = () => {
  ...
  return (
    <div className="mt-12 px-4 sm:px-6 lg:px-8 animate-fadeIn">
      <h3 className="text-2xl font-bold text-tropical-teal mb-6">🎯 Matching Jobs For You</h3>
      ...
      <div className="space-y-8">
        {recommendedJobs.map((job, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white border border-ash-grey rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-6"
            >
              ...
              <div className="mt-6 text-sm text-gray-700">
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-tropical-teal transition-all"
                      style={{ width: `${score * 100}%` }}
                    ></div>
                  </div>
                  <p className="text-xs text-tropical-teal mt-1">{(score * 100).toFixed(1)}% match</p>
                </div>
                  ...
                  {skillGap.length > 0 ? (
                    <ul className="flex flex-wrap gap-2">
                      {skillGap.map((skill, i) => (
                        <li
                          key={i}
                          className="px-3 py-1 rounded-full bg-vibrant-coral/10 text-vibrant-coral text-xs border border-vibrant-coral/20"
                        >
                          {skill}
                        </li>
                      ))}
                    ...
                    <p className="text-sm text-tropical-teal">✅ No skill gaps – You're ready to apply!</p>
                  )}
                </div>
              </div>
            </div>
