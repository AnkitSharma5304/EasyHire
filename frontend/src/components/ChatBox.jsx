...
const ChatBox = () => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-soft-linen text-slate-800 px-4 sm:px-8 py-6 animate-fadeIn">
        
        <div className="max-w-5xl mx-auto mb-6">
          <h2 className="text-2xl font-bold text-tropical-teal">
            Chat for:{" "}
        ...
        <div className="max-w-5xl mx-auto h-[60vh] overflow-y-auto border border-ash-grey bg-white rounded-xl p-4 space-y-4 shadow">
          {messages.map((msg) => {
            const isSender = msg.senderId?.toString() === senderId;
            const bubbleColor = isSender
              ? "bg-tropical-teal/10 text-tropical-teal"
              : "bg-white border border-ash-grey text-gray-800";
                ...
                <div className={`max-w-[75%] px-4 py-3 rounded-2xl ${bubbleColor} shadow-md`}>
                  ...
                  {msg.mediaUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="block mt-2 text-tropical-teal underline"
                    >
                      📎 View Attachment
                    </a>
                  )}
        ...
        <div className="max-w-5xl mx-auto mt-6 flex flex-col sm:flex-row gap-4 items-center">
          <input
            ...
            className="flex-1 p-3 rounded-md bg-white text-slate-800 border border-ash-grey placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-tropical-teal"
          />

          
          <div className="flex flex-col w-full sm:w-64">
            <label className="relative cursor-pointer text-center">
              <span className="flex items-center justify-center gap-2 bg-tropical-teal hover:bg-tropical-teal/90 text-white py-2 px-4 rounded-md transition">
          ...
          <button
            ...
            className="bg-vibrant-coral hover:bg-vibrant-coral/90 text-white px-6 py-3 rounded-md font-medium transition duration-200 disabled:bg-slate-400"
          >
            Send
          </button>
        </div>
